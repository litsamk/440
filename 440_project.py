...
# Print Python executable and version for debugging purposes
...
def load_data():
    ...
    Returns:
        Tuple containing expression data, sample data, and drug data.
    """
...
def preprocess_data(expression, sample_data, drug_data):
    """
    Preprocess the data to create a normalized AnnData object with proper drug metadata.
    """
    ...
    print(f"After sample merge: {len(sample_merged)} samples")

    # Second merge: Drug metadata
    sample_merged = pd.merge(
        sample_merged,
        drug_data,
        left_on='Drug',
    ...
    if 'Drug class' not in adata.obs.columns:
        print("WARNING: 'Drug class' missing after all merges!")
    else:
